...
import { generateTestProfiles } from "@/utils/constants/testProfiles";

export default async function AdminDrawPage() {
	// For now, always use test profiles
	const profiles = generateTestProfiles() as Profile["Row"][];

	// Create entries array: each profile gets one entry per ticket
	const entries: Array<{ name: string; avatar: string; tickets: number }> = [];
	...
	// Shuffle entries for randomization
					...
					<div className="rounded-[40px] border border-white/50 bg-white/90 px-6 py-8 shadow-2xl backdrop-blur-xl md:px-10 md:py-10 ring-1 ring-emerald-900/5 relative overflow-hidden">
						...
						<header className="mb-8 flex flex-col items-center gap-3 relative">
							...
							<p className="max-w-xl text-center text-sm font-medium text-emerald-800/70 leading-relaxed">
								{entries.length > 0
									? `${entries.length} ticket${entries.length !== 1 ? "s" : ""} in the draw`
									: "No tickets in the draw yet"}
							</p>
						</header>

						{/* Wheel component */}
						{entries.length > 0 ? (
							<PrizeDraw entries={entries} />
						) : (
							<div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-6 py-12 text-center">
								<p className="text-emerald-800">No profiles with tickets yet.</p>
							</div>
						)}
